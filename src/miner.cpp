...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("NATUMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("NATUMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("NATUMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("natu-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running NATUMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
