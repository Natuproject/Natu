...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='natu.conf'
CONFIGFOLDER='/root/.natu'
COIN_DAEMON='natud'
COIN_CLI='natu-cli'
COIN_PATH='/usr/local/bin/'
COIN_REPO='https://github.com/natu-dev/natu-core.git'
COIN_TGZ='https://github.com/natu-dev/natu-core/releases/download/3.0.0/natu-3.0.0-x86_64-linux-signed.tar.gz'
...
function get_ip() {
  declare -a NODE_natu
  for natu in $(netstat -i | awk '!/Kernel|Iface|lo/ {print $1," "}')
  do
    NODE_natu+=($(curl --interface $natu --connect-timeout 2 -s4 icanhazip.com))
  done
